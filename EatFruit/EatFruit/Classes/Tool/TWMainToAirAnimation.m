...
@interface TWMainToAirAnimation ()
@property (nonatomic,assign) BOOL isPresented;// 标记是否是弹出
@end

@implementation TWMainToAirAnimation

#pragma mark --init
+ (TWMainToAirAnimation *)shareMainToAirAnimation{
    static TWMainToAirAnimation * animation = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        if (!animation) {
            animation = [[TWMainToAirAnimation alloc]init];
        }
    });
    return animation;
}
