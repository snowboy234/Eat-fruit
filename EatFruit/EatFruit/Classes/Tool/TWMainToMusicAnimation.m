...
@interface TWMainToMusicAnimation ()
@property (nonatomic,assign) BOOL isPresented;// 标记是否是弹出
@end

@implementation TWMainToMusicAnimation

#pragma mark --init
+ (TWMainToMusicAnimation *)shareMainToMusicAnimation{
    static TWMainToMusicAnimation * animation = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        if (!animation) {
            animation = [[TWMainToMusicAnimation alloc]init];
        }
    });
    return animation;
}
...
- (void)animationForPresentedView:(id<UIViewControllerContextTransitioning>)transitionContext{
    ...
    UIImageView * imageView = [_delegate getBottomScreenImage];
